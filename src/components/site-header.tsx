...
import { SERVICE_CLOSED, siteConfig } from "@/lib/site";
...
export function SiteHeader({ locale = "en" }: { locale?: SiteLocale }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/80 backdrop-blur-md">
      {/* Every page says it, not just the one with the form on it. */}
      {SERVICE_CLOSED ? (
        <p className="bg-foreground px-5 py-2 text-center text-xs text-background sm:px-8">
          {dict.closed.heading} — {dict.closed.body}
        </p>
      ) : null}
